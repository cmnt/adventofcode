...
const resultPart2 = (puzzle) => getMissingSeats(puzzle);
...
const getRow = (rowCode) => rowCode.split('').reduce((range, code) => {
...
const getColumn = (columnCode) => columnCode.split('').reduce((range, code) => {
  const diff = (range[1] - range[0] + 1) / 2;
  if (code === 'L') {
    return [range[0], range[1] - diff];
  }
